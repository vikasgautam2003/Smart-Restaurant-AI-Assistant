        ...
        conf = conf
            ? Object.assign(Object.assign({}, conf), { multiVector: conf.multiVector
                    ? Object.assign(Object.assign({}, conf.multiVector), { encoding: conf.multiVector.encoding
                            ? Object.assign(Object.assign({}, conf.multiVector.encoding), opts.encoding) : opts.encoding }) : index_js_1.vectorIndex.multiVector.multiVector({ encoding: opts.encoding }) }) : {
            multiVector: index_js_1.vectorIndex.multiVector.multiVector({ encoding: opts.encoding }),
            type: 'hnsw',
        };
    }
    if (opts === null || opts === void 0 ? void 0 : opts.quantizer) {
        if (!conf) {
            conf = index_js_1.vectorIndex.hnsw({ quantizer: opts.quantizer }).config;
        }
        if (parsing_js_1.VectorIndexGuards.isDynamic(conf)) {
            conf.hnsw = conf.hnsw
                ? Object.assign(Object.assign({}, conf.hnsw), { quantizer: opts.quantizer }) : index_js_1.vectorIndex.hnsw({ quantizer: opts.quantizer }).config;
            conf.flat = conf.flat
                ? Object.assign(Object.assign({}, conf.flat), { quantizer: opts.quantizer }) : index_js_1.vectorIndex.flat({ quantizer: opts.quantizer }).config;
        }
        else {
